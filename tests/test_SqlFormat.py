...
def test_select_generate():
    select_sql, select_value = AboutSql.select_generate(db_table="student", key=["id"], rule={"age|=": 18, "sex|!=": "male"}, limit=1)
...
def test_insert_generate():
    insert_sql, insert_value = AboutSql.insert_generate(db_table="student", data={"name": "zhangsan", "age": 18})
...
def test_update_generate():
    update_sql, update_value = AboutSql.update_generate(db_table="student", data={"score": 4}, rule={"name": "zhangsan"})
