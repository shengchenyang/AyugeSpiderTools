...
def test_match_gap():
    _left_offset = 195
    _right_offset = 210
    gap_distance = CvnpilKit.match_gap(
    ...
    gap_distance = CvnpilKit.match_gap(target_bytes, template_bytes)
    assert _left_offset <= gap_distance <= _right_offset
...
def test_discern_gap():
    _left_offset = 212.5
    _right_offset = 217.5
    # 参数为图片全路径的情况
    gap_distance = CvnpilKit.discern_gap(
    ...
    # 参数为图片 bytes 的情况
    bg_img_bytes = Path(tests_dir, "docs/image/2.jpg").read_bytes()
    slider_img_bytes = Path(tests_dir, "docs/image/1.png").read_bytes()
    gap_distance = CvnpilKit.discern_gap(
        bg_img_bytes, slider_img_bytes, f"{tests_dir}/docs/image/33.png"
    )
    assert _left_offset <= gap_distance <= _right_offset
