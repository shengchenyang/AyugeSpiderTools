...
def test_match_img_get_distance():
    distance_res = VerificationCode.match_img_get_distance("doc/image/new_target.jpg", "doc/image/new_template.png")
    print(f"滑块缺口位置1: {distance_res}")

    with open("doc/image/new_target.jpg", "rb") as f:
        target_bytes = f.read()
    with open("doc/image/new_template.png", "rb") as f:
...
def test_get_yidun_gap():
    # 参数为图片全路径的情况
    tracks = VerificationCode.get_yidun_gap("doc/image/1.png", "doc/image/2.jpg", "doc/image/3.png")
    ...
    with open("doc/image/2.jpg", "rb") as f:
        template_bytes = f.read()
    tracks = VerificationCode.get_yidun_gap(target_bytes, template_bytes, "doc/image/33.png")
