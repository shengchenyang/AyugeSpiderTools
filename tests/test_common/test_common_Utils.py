...
def test_extract_with_json_rules():
    res = ToolsForAyu.extract_with_json_rules(json_data=json_data_example, query_rules=[["data", "im"]])
    print("res:", res)

    # 参数层级大于 2 时，会报错
    try:
        res1 = ToolsForAyu.extract_with_json_rules(json_data=json_data_example, query_rules=[[["depth_to_big"], "data", "im"]])
        print("res:", res1)
    except Exception as e:
        print("err:", e)

    res2 = ToolsForAyu.extract_with_json_rules(json_data=json_data_example, query_rules=[["data", "yy"], "message"])
    print("res2:", res2)

    # 当 json 字段的值为 int, float, str, complex 或 list 等没有 get 方法的时候
    try:
        res3 = ToolsForAyu.extract_with_json_rules(json_data=json_data_example, query_rules=[["data", "im", "cc"], "message"])
        print("res3:", res3)
    except Exception as e:
