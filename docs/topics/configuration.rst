...
.. note::
   ...
   disable_oob 是一个默认 false 的布尔值，用于控制是否禁用 Oracle 的 Out-of-Band Break(OOB) 功能。该参数仅在 python-oracledb 的 \
   Thin 模式下生效。
...
用于在使用 AyuRabbitMQSpider 时定义对应的任务队列信息。

.. csv-table::
   :header: "参数名", "参数备注", "描述"
   :widths: 10, 15, 30

   "name", "_", "当前 spider 名称，只用于记录而已"
   "task_mq", "_", "当前 spider 的任务队列的配置名，不是指队列名"
   "result_mq_queue1", "_", "这里是自定义参数，只是表明用户可以用于推送结果到不同队列，用于以 mq 为\
   基础的项目。"
   "result_mq_queue2", "_", "同 result_mq_queue1"

.. note::

   配置中的 task_mq 就是指对应的任务队列的配置名，比如 task_mq 设置为 demo_mq_task_mq，那么就需要在\
   .conf 中配置 [demo_mq_task_mq]，而且其中的配置参数和 [mq] 中的一致。
